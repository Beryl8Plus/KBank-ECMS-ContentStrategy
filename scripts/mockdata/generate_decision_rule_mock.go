...
func buildRedisSeedScript(attributes []attributeDef, users []userSeed) string {
	...
	buf.WriteString("#\n")
	buf.WriteString("# Key format  : customer_profile:CIS_ID:{cisID}\n")
	...
	buf.WriteString("  $REDIS_CLI SET \"customer_profile:CIS_ID:${CIS_ID}\" \"${PAYLOAD}\"\n")
	buf.WriteString("  echo \"  SET customer_profile:CIS_ID:${CIS_ID}\"\n")
	buf.WriteString("}\n")
	buf.WriteString("\n")
