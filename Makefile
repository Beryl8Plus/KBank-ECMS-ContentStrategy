...
db-mock-data-sql-up:
	@docker exec $(POSTGRES_CONTAINER) psql -q -U $${DB_USER:-postgres} -d $${DB_NAME:-kbank_ecms} -c \
		"CREATE TABLE IF NOT EXISTS $(GOOSE_MOCK_TABLE) (id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, version_id BIGINT NOT NULL, is_applied BOOLEAN NOT NULL, tstamp TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT now());"
	@docker exec $(POSTGRES_CONTAINER) psql -q -U $${DB_USER:-postgres} -d $${DB_NAME:-kbank_ecms} -c \
